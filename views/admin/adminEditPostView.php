...
<?php ob_start(); ?>
	...
	<form action="index.php?action=changePost&amp;id=<?= $post->id(); ?>" method="post">
		<label for="titre"> Titre du billet <input type="text" name="titre" id="titre" value="<?php echo $post->titre(); ?>" required /> </label> <br>
		<label class="labelEditPost"> <textarea class="tiny" id="textareaEditPost" name="contenu"> <?php echo $post->contenu(); ?> </textarea> </label> <br>
